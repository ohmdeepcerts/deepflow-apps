...
import {
  fuzzyAddr, selectAddr, closeAddrDrop, postcodeLookup, confirmPostcode, closeAllAutofillDrops, smartAutofill,
} from './jobs-address-autofill.js';
import {
  checkDuplicatePhone, dupUseExisting, dupUpdateName, _resolveLandlordPerson, _resolveAgency,
  saveLandlordFromJob, saveAgencyFromJob, saveAgentFromJob, sendLandlordWA,
} from './jobs-directory-sync.js';
import {
  showWaPanel, waShowEng, waSingleJobById, waSingleJob, waJobsSelected, openWhatsApp, sendToWA, copyText, copyWAText,
  waSingleEngJob, waEngineerAllJobs,
} from './jobs-whatsapp.js';
// Re-exported (not just imported) because directory-crud.js still imports
// sendToWA from main.js — its own import list didn't change when this split
// moved sendToWA's implementation to jobs-whatsapp.js.
export { sendToWA } from './jobs-whatsapp.js';
import {
...
import {
  setJobsView, initDragDrop, getColTemplate, renderJobsHeader, applyColTemplate,
...
import {
  ...
  updatePriDotsVisibility, handlePriDotClick, setPriFilter,
} from './jobs-bulk-select.js';
...
// their own import lists didn't change when this split moved the
// implementations to invoice-documents.js.
export {
  _sendEmail, _brandedEmailShell, _certReadyEmailHtml, _certLockedEmailHtml,
  signedUrl, downloadInvPDFById, generateAndStoreInvoicePDF,
} from './invoice-documents.js';
import {
  bulkMarkPaid, createRecurringInv, saveAndSendInv, deleteDuplicateInvoices, duplicateInv,
  exportInvsCSV, previewWaTemplate, copyWaTemplate, sendAllOverdueWA,
} from './invoices-misc-actions.js';
// Re-exported (not just imported) because jobs-columns-dragdrop.js still
// imports clearSel from main.js — its own import list didn't change when
// this split moved clearSel's implementation to jobs-bulk-select.js.
export { clearSel } from './jobs-bulk-select.js';
...
import { openBroadcast, _ALERTS_SQL, _ensureAlertsTable, sendBroadcast, showAlertSetupModal } from './broadcast-alerts.js';
import {
...
} from './pl-dashboard.js';
import { _sendPLReminder, exportPLCSV, exportXeroCSV, exportQuickBooksCSV, _downloadCsv, escCsv } from './pl-exports.js';
import {
...
import {
  _portalBaseUrl, _buildPortalUrl, _jobPortalLink, shareClientPortal, copyClientPortal,
...
export const _supaAuth = createSupaAuthClient();
...
export const queueableSave = _officeQueue.queueableSave;
...
// ════════════════════════════════════════════════════════════════
//  CLIENT PORTAL SHARING/INVITE/CONTACTS
//  Moved to client-portal-admin.js (Phase 1 modularization) —
//  _portalBaseUrl/_buildPortalUrl/_jobPortalLink included; certs.js and
//  audit.js's imports of the latter two were repointed there too.
// ════════════════════════════════════════════════════════════════
...
export let _jobCache=null;
export function _setJobCache(v){ _jobCache = v; }
let _jobCacheTs=0;
let _invType='all',_invNavMode='dashboard',_invSaveTimer={};
let _pendingSaves=0;
export let _lastOpenedJob=null;

export let editJid=null,selJobs=new Set(),allProps=[];
let _jobSortCol='created';
...
// Priority-dot filter state — read by renderJobs(), written by setPriFilter()
// (jobs-bulk-select.js) and applySavedView() (both below).
export let _priFilter='';
export function _setPriFilterState(v){ _priFilter = v; }
...
export let _calPaneVisible=true;
let _jUnbookedOnly=false,_jUnconfirmedOnly=false;
...
// Shared with the Client Portal's PIN screens and "no link" landing screen —
// see packages/ui/network-canvas.js. Only the show/hide lifecycle (tied to
// #pin-overlay's visibility) stays local, since that's Office-specific.
function initLoginCanvas(){
  const canvas=document.getElementById('login-canvas');
  if(!canvas) return;
  const {start,stop}=initNetworkCanvas(canvas,{
    sizeCanvas(){
      canvas.width=Math.max(canvas.parentElement?.offsetWidth||0,window.innerWidth*.7,800);
      canvas.height=Math.max(canvas.parentElement?.offsetHeight||0,window.innerHeight,600);
    }
  });
  ...
  window.addEventListener('resize',()=>{stop();if(overlay.style.display!=='none')start();});
...
// job).
export async function _concurrentEach(items, fn, concurrency=8){
  let i=0;
...
export function _invalidateJobCache(){
  _jobCache = null;
  _jobCacheTs = 0;
}
...
export function _renderJobsKeepScroll(){
...
export async function _applyStatusChange(id,status,opts){
...
export function getVatRate(){return officeVatRate(S);}
...
export async function openJobModal(id){
  editJid=id||null;
...
export async function renderInvKPIs(){
  renderInvSubnavKPIs();
...
export function _invalidateCache(store){
...
export async function openInvSendModal(id){
  curInvId=id;
  const inv=await dGet('invoices',id);
...
// ════════════════════════════════════════════════════════════════
//  TEAM / ENGINEER ACCESS MANAGEMENT
//  Moved to team.js and engineer-access.js (Phase 1 modularization).
//  addAccessRow/addPropRow below are unrelated Settings row-adders that
//  just happened to sit in this part of the file; changeUserRole and what
//  follows are a separate, still-live legacy roster system (S.users-based)
//  distinct from the Supabase-Auth-based team those two files cover.
// ════════════════════════════════════════════════════════════════
...
//  SETTINGS TABS
// ════════════════════════════════════════════════════════════════
// Portal Contacts CRUD moved to client-portal-admin.js — loadPortalContacts
// is imported back below since switchSetTab calls it directly.
...
// ════════════════════════════════════════════════════════════════
// Panel UI, unread badge, and _pushNotif itself now live in
// notifications-panel.js. Polling + Realtime sync live in
// jobs-realtime-sync.js (Phase 5e-2) — only the badge helpers stay here,
// since _sb() itself calls them defensively on every request.
...
export function _setLiveBadge(state, label){
...
export function _setSyncing(){ _setLiveBadge('syncing','Syncing…'); }
export function _setSynced() { _setLiveBadge('live','Live'); }
export function _setOffline(){ _setLiveBadge('offline','Offline — check connection'); }

// Show "Synced" flash after each save
export function _flashSynced(label='✓ Synced'){
...
// createAllTables (right after this) still needs it.
...
export async function saveInvWithJobSync(){
...
// Portal Invite Modal (visiting card, canvas background, QR, PNG download)
// moved to client-portal-admin.js along with everything else portal-related.

/* ════════════════════════════════════════
   P&L DASHBOARD — Company Finance
   Moved to pl-dashboard.js (dashboard/tabs/renderers) and pl-exports.js
   (CSV/Xero/QuickBooks exports, _sendPLReminder) — Phase 1 modularization.
   ════════════════════════════════════════ */
...
Object.assign(window, {
  ...
  toggleUnassignedView, toggleUserMenu, unlockPaidInv, updateApplianceField, updateCertAddrSugg, updateCertJobSugg, updateCreditItem, updateEngPerm, updateLogo, updatePortalContact, updInvTotals,
