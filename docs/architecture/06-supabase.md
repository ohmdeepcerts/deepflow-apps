...
- **One bucket:** `deepflow` (`storage.buckets`, live-queried: `public = false`, no `file_size_limit`, no `allowed_mime_types` restriction).
- **Folder convention** (from the real upload call sites, not guessed): `jobs/<jobid>/<timestamp>-<rand>.<ext>` for job photos (`apps/engineer/photos.js`), `certs/<certid>/<filename>` for certificate PDFs (`apps/office/certs.js`), `invoices/<invid>/<number>.pdf` for invoice PDFs (`apps/office/main.js`).
...
- **Downloads are now RLS-gated too — the bucket was public until this same session found it and fixed it.** It was `public = true` when this document was first drafted: per Supabase's own documentation, a public bucket "bypasses access controls for both retrieving and serving files... anyone who possesses the asset URL can readily access the file," and every upload helper across all three apps returned exactly that public URL form after upload — no signed URL anywhere. That was closed in the same session, while the bucket held zero objects: flipped to `public = false` (migration `20260808_make_deepflow_bucket_private.sql`), and Office/Engineer (which both hold a real session) now call `signedUrl()` to sign a short-lived URL themselves, authorized by the `deepflow_staff_select` policy above. The Client Portal has no session to do that with, so it calls a new Edge Function, `portal-sign-url` (Section 7), which re-resolves the visitor's identity server-side — the same way `portal_get_certs`/`portal_get_attachments`/`portal_get_invoices` already do — and only signs a path if it's genuinely found among that identity's own records.

See [09-storage.md](09-storage.md) for the full mechanism, including per-app expiry durations and the reasoning behind them.
...
Seven functions, all `ACTIVE` (confirmed via `list_edge_functions` against the live project — `portal-sign-url` was added in the same session that made the storage bucket private, see Section 5 above). Full request/response contracts belong in [`docs/api/15-apis.md`](../api/15-apis.md) (a sibling doc covering all API surfaces, not written yet as of this document) — this section names each function, its purpose, and its auth model only.
...
| `portal-sign-url` | false | Manual: re-resolves the caller's `(type, id)` identity server-side via the same `portal_get_jobs`/`portal_get_certs`/`portal_get_attachments`/`portal_get_invoices` RPCs the Portal already trusts, using the service role | The bridge that lets the unauthenticated Client Portal read files from the now-private `deepflow` bucket: given `{type, id, paths}`, it rebuilds the set of storage paths that identity's own certs/attachments/invoices actually reference, and signs only the requested paths that are genuinely in that set — a guessed or adjacent path is silently dropped, never signed. Added the same session the bucket went private (see Section 5). Source: `supabase/functions/portal-sign-url/index.ts`. |
